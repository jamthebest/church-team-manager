...
const ResultTab: React.FC<ResultsTabProps> = ({ competitions, teams }) => {
  ...
  const filteredCompetitions = competitions.filter((competition) => {
    const isTeamFilterSatisfied = teamFilter ? competition.teams.includes(teamFilter) : true;
    const isFilterTypeRule = filterType ? competition.type === filterType : true;
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          <label htmlFor="teamFilter" className="block mb-1">Filtrar por Equipo:</label>
          <select
            id="teamFilter"
            value={teamFilter}
        ...
        <div>
          <label htmlFor="filterType" className="block mb-1">Filtrar por Tipo:</label>
          <select
            id="filterType"
            value={filterType}
              ...
              <tr key={competition.id} className="border-b">
                ...
                <td className="p-2">
                  <div className="flex flex-wrap items-center gap-2">
                    {competition.teams.map((teamId, index) => {
                      const team = getTeamById(teamId);
                      return (
                        <div key={teamId} className="flex items-center space-x-1">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: team?.color }}>
                            {competition.scores[index]}
                          </div>
                          <span>{team?.name}</span>
                        </div>
                      );
